...
class Kitsune(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        #print('tail loss shape is', tails_losses.shape)
        ...
        #print(f'reconstructed_loss_dist size is {reconstructed_loss_dist.shape}')
    ...
    def reconstruct(self,x: torch.Tensor):
        ...
        reconstructed_feat = []
        for i, (feat, tail) in enumerate(zip(split_feat, self.tails)):
            reconstructed_feat_partial = tail(feat)
            reconstructed_feat.append(reconstructed_feat_partial)
        return (split_feat, reconstructed_feat)
