...
class TabularAID(Interpreter):
    ...
    def __init__(self, model, thres, 
                 ...
                 lr=0.01,                 # learning rate of optimizing reference (*within auto_params)
    ...
    def forward(self, anomaly, MIN=-np.Inf, MAX=np.Inf):
        ...
        anomaly = anomaly.to(self.device)
#         reference = anomaly.clone().detach()
        ...
        for step in range(self.steps):
            ...
            ora, re = self.model.reconstruct(w)
            re = self.restore11(re)
            ora = self.restore11(ora)
            bounded_w = torch.min(torch.max(w,MIN), MAX)
            dist = Dist(re,bounded_w)
           ...
           # print(loss1, loss2)
            ...
            if Last_Loss - Loss < 0.00001 and self.early_stop:
                # print('')
                # end_time = time.time()
        ...
        return {'index_inc':clip_index.numpy(), 'value_inc':reference[clip_index].numpy()}, loss_set
        
    def restore11(self, x):
        y = []
        for torsor_ in x:
            for i in torsor_.flatten():
                y.append(i)
        #print(torch.tensor(y).shape)
        return torch.tensor(y,requires_grad=True).to('cuda:6')
    ...
    def show_table(self, anomaly, interpretation, normer):
        ...
        pseudo_value = np.asarray([0]*len(anomaly))
        pseudo_value[index_inc] = value_inc
        value_inc = pseudo_value[index_inc]
        anomaly = anomaly[index_inc]
        reference = value_inc
